...
const SaucerGame = () => {
  ...
  const [renderEntities, setRenderEntities] = useState({
    saucer: { x: 350, y: 100, width: 80, height: 40 },
    cows: [],
    farmers: [],
    bullets: []
  });
  ...
  const gameLoop = () => {
    ...
    setIsBeaming(isBeamingRef.current);
    setRenderEntities({
      saucer: { ...saucer },
      cows: [...cows],
      farmers: [...farmers],
  ...
  return (
    <div className="saucer-game-container" style={{ width: GAME_WIDTH, height: GAME_HEIGHT }}>
      ...
      <div className="game-world">
        ...
        <div
          className={`entity saucer ${isBeaming ? 'shaking' : ''} ${crashed ? 'crashed' : ''}`}
          style={{
            left: renderEntities.saucer.x,
            top: crashed ? undefined : renderEntities.saucer.y,
            width: renderEntities.saucer.width,
            height: renderEntities.saucer.height,
            '--crash-start-y': `${renderEntities.saucer.y}px`
          }}
        >
          🛸
          {isBeaming && !crashed && <div className="tractor-beam"></div>}
        </div>

        {renderEntities.cows.map(cow => (
        ...
        {renderEntities.farmers.map(farmer => (
        ...
        {renderEntities.bullets.map((bullet, i) => (
        ...
        <div className="mobile-controls">
          <div className="d-pad">
            ...
            <div className="horizontal">
              <button onTouchStart={(e) => handleTouchStart(e, 'ArrowLeft')} onTouchEnd={() => handleTouchEnd('ArrowLeft')}>⬅️</button>
              <button onTouchStart={(e) => handleTouchStart(e, 'ArrowRight')} onTouchEnd={() => handleTouchEnd('ArrowRight')}>➡️</button>
            </div>
            <button onTouchStart={(e) => handleTouchStart(e, 'ArrowDown')} onTouchEnd={() => handleTouchEnd('ArrowDown')}>⬇️</button>
          </div>
          <div className="action-btn">
            <button
              className={isBeaming ? 'active' : ''}
